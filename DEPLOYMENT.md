...
| `HEROKUMCP_MASTER_KEY` | yes | Auto-generated by `app.json`. 32 random bytes base64-encoded; encrypts user OAuth tokens at rest. DO NOT change after first deploy or stored tokens become unrecoverable. |
| `HEROKUMCP_PUBLIC_URL` | yes (Phase 4.5+) | Externally-resolvable base URL of this server, no trailing slash. Used as the `issuer` in OAuth 2.1 metadata documents and as the base for redirect URIs. Example for Heroku deploys: `https://my-team-heroku-mcp.herokuapp.com`. For local development: `http://localhost:3000`. Must be HTTPS in production — Claude Desktop will not complete OAuth flows against HTTP URLs other than `localhost`. |
...
---
## Connecting clients
After your server is deployed and running, there are several supported paths to connect Claude clients to it. Pick the one that matches your client.
### Claude Desktop (Custom Connector — recommended, Phase 4.5+)
Best for end users with Claude Desktop on Pro, Max, Team, or Enterprise plans. Requires Phase 4.5 OAuth provider support to be live on the server.
1. In Claude Desktop, open Settings → Connectors → Add custom connector.
2. Name: anything memorable (e.g. "Heroku Platform").
3. URL: `https://<your-server>/mcp`
4. Leave OAuth Client ID and Secret blank — Claude Desktop registers itself automatically via Dynamic Client Registration.
5. Click Connect. Your browser opens to your server's sign-in page, you authenticate with Heroku, and Claude Desktop is connected.
After connecting, you can manage the connection from `/me` on your server — each connected client shows up with a Revoke button.
### Claude Code (CLI)
Best for developer workflows. Uses a bearer token issued once.
1. In your browser, go to `https://<your-server>/` and sign in with Heroku. Copy the `hmcp_...` token shown on `/me`.
2. In your terminal:

   ```bash
   claude mcp add heroku-platform \
     --transport http \
     --header "Authorization: Bearer hmcp_..." \
     https://<your-server>/mcp
   ```

3. Start Claude Code (`claude`) and the MCP server's tools are available.

The bearer token never expires until you click "Sign out everywhere" on `/me`.
### MCP Inspector, curl, or custom client
Same bearer token from `/me`. Send `Authorization: Bearer hmcp_...` with each request. See the MCP spec for the protocol details.
### Claude Desktop on Free plan (no Custom Connector access)
Use the `mcp-remote` bridge. In `~/Library/Application Support/Claude/claude_desktop_config.json`:
```json
{
  "mcpServers": {
    "heroku-platform": {
...
```
Use the bearer token from `/me`, same as the Claude Code path. Quit and reopen Claude Desktop to load the connector.
### Local development note
When developing locally with `HEROKUMCP_PUBLIC_URL=http://localhost:3000`:
- **Claude Code path works fully.** It accepts `http://localhost` for bearer-auth requests.
- **Claude Desktop Custom Connector requires HTTPS.** For end-to-end OAuth testing locally, use `ngrok http 3000` or similar to get an HTTPS URL, then set `HEROKUMCP_PUBLIC_URL` to the ngrok URL.
- **The Phase 4.5 OAuth provider tests** can simulate Claude's role with curl entirely over HTTP. Only the final smoke test against actual Claude Desktop requires HTTPS.
---
...
7. **Dynamic Client Registration is open (Phase 4.5+).** Anyone who can reach `/oauth/register` can register an OAuth client. This is by design — registering as a client without a valid allowlisted user behind it grants no access. The actual security boundary remains `MCP_ALLOWED_EMAILS` / `MCP_ALLOWED_TEAMS` at sign-in time.
---
...
**New for Phase 4.5:** `HEROKUMCP_PUBLIC_URL` is required. It's the
externally-resolvable base URL of the server, used in OAuth 2.1 metadata
documents (the `issuer` field) and as the base for OAuth redirect URIs. For
a Heroku deployment, set it to `https://<app-name>.herokuapp.com`. Phase 4
deployments don't need this var; Phase 4.5 and later do.
