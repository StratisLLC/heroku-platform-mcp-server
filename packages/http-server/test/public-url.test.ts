...
describe('PublicUrlResolver', () => {
  it('returns explicit value when set', () => {
    const r = new PublicUrlResolver({ explicit: 'https://example.com', isProduction: true, port: 3000 });
  ...
  it('explicit value cannot be overridden by request', () => {
    const r = new PublicUrlResolver({ explicit: 'https://configured.com', isProduction: true, port: 3000 });
...
describe('publicUrlMiddleware + .well-known integration', () => {
  ...
  it('serves oauth-authorization-server with the URL resolved from the first request', async () => {
    ...
    const res = await app.request('/.well-known/oauth-authorization-server', {
      headers: { host: 'localhost:3000', 'x-forwarded-host': 'myapp.herokuapp.com', 'x-forwarded-proto': 'https' },
    });
