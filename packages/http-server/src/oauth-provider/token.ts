...
async function handleAuthorizationCode(
  ...
  if (auth.redirectUri !== b.redirect_uri) {
    return c.json(
      { error: 'invalid_grant', error_description: 'redirect_uri does not match the authorization' },
      400,
    );
  }
...
async function handleRefreshToken(
  ...
  if (row.refreshExpiresAt.valueOf() <= Date.now()) {
    return c.json(
      { error: 'invalid_grant', error_description: 'refresh_token has expired' },
      400,
    );
  }
...
async function readForm(c: import('hono').Context<AppEnv>): Promise<FormData | null> {
