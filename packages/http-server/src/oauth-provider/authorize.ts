...
export function buildAuthorizeRoutes(deps: AuthorizeDeps): Hono<AppEnv> {
  ...
  router.get('/oauth/authorize', async (c) => {
    ...
    if (client.revokedAt) {
      return c.json({ error: 'invalid_client', error_description: 'client has been revoked' }, 400);
    }
  ...
  router.post('/oauth/consent', async (c) => {
    ...
    const decision = formField(form, 'decision');
    const clientId = formField(form, 'client_id');
    const redirectUri = formField(form, 'redirect_uri');
    const codeChallenge = formField(form, 'code_challenge');
    const codeChallengeMethod = formField(form, 'code_challenge_method');
    const state = formFieldOptional(form, 'state');
    const scope = formFieldOptional(form, 'scope');
...
function currentAuthorizeUrl(c: Context<AppEnv>): string {
