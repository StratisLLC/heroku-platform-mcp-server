...
describe('account-tier writes', () => {
  ...
  it('account_sms_number_recover POSTs to the recover action', async () => {
      ...
      responses: [
        {
          match: (url, init) =>
            ...
            init?.method === 'POST',
          body: { id: 'sms-1', phone_number: '+15555550100' },
        },
      ],
    });
    const result = (await client.callTool({
    ...
    // The 2FA phone number is stripped from the response.
    const env = parseEnvelope<Record<string, unknown>>(result);
    expect(env.ok).toBe(true);
    expect(env.data).not.toHaveProperty('phone_number');
    expect(env.data).toHaveProperty('id', 'sms-1');
  });
