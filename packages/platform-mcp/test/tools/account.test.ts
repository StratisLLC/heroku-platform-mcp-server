...
describe('account-tier tools', () => {
  ...
  it('oauth_clients_list strips the cleartext client secret', async () => {
    const { client } = await spinUpServer({
      capabilities: accountOnly,
      responses: [
        {
          match: (url) => url === 'https://api.heroku.com/oauth/clients',
          body: [{ id: 'c-1', name: 'app', secret: 'super-secret', redirect_uri: 'https://x/cb' }],
        },
      ],
    });
    ...
    expect(env.ok).toBe(true);
    expect(env.data?.[0]).not.toHaveProperty('secret');
    expect(env.data?.[0]).toHaveProperty('id', 'c-1');
  });

  it('oauth_clients_info strips the cleartext client secret', async () => {
    const { client } = await spinUpServer({
      capabilities: accountOnly,
      responses: [
        {
          match: (url) => url === 'https://api.heroku.com/oauth/clients/c-1',
          body: { id: 'c-1', name: 'app', secret: 'super-secret' },
        },
      ],
    });
    const result = (await client.callTool({
      name: 'oauth_clients_info',
      arguments: { id: 'c-1' },
    })) as { content: unknown[] };
    const env = parseEnvelope<Record<string, unknown>>(result);
    expect(env.ok).toBe(true);
    expect(env.data).not.toHaveProperty('secret');
    expect(env.data).toHaveProperty('name', 'app');
  });
