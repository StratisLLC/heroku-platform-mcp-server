...
describe('teams-tier writes', () => {
  ...
  it('teams_create description surfaces the standalone/enterprise context', async () => {
    const { client } = await spinUpServer({ capabilities: teamsOnly });
    ...
    expect(create?.description).toContain('enterprise_account_teams_create');
    expect(create?.description).toMatch(/[Ee]nterprise/);
    expect(del?.description).toContain('standalone');
    expect(del?.description).toMatch(/[Ee]nterprise/);
  });
