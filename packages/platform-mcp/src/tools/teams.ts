...
export function registerTeamsTools(server: McpServer, ctx: ToolContext): void {
  ...
  server.registerTool(
    'team_daily_usage',
    {
      ...
      description:
        'Daily usage breakdown for a team across a date window. Wraps GET /teams/{id_or_name}/usage/daily.',
  ...
  server.registerTool(
    'team_monthly_usage',
    {
      ...
      description:
        'Monthly usage breakdown for a team across a date window. Wraps GET /teams/{id_or_name}/usage/monthly.',
      inputSchema: teamAndUsageRangeInput,
      annotations: { readOnlyHint: true, openWorldHint: true },
    },
    ({ team, start, end }) =>
