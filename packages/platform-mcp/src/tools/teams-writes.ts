...
export function registerTeamsWriteTools(server: McpServer, ctx: ToolContext): void {
  ...
  registerWriteTool<typeof teamsCreateShape, HerokuRecord>(server, ctx, {
    ...
    title: 'Create team',
    description:
      'Create a standalone (non-enterprise) team. Wraps POST /teams. Heroku now recommends creating teams via an Enterprise account (enterprise_account_teams_create); this endpoint still works but yields a team with limited functionality compared to enterprise-owned teams — use it only when you specifically want a standalone team.',
