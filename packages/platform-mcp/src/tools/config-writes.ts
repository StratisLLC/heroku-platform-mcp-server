...
export function registerConfigWriteTools(server: McpServer, ctx: ToolContext): void {
  registerWriteTool<typeof configVarsUpdateShape, HerokuRecord>(server, ctx, {
    ...
    description:
      'Update or delete config vars on an app. Pass null as a value to delete a key. Heroku triggers a new release on success. Wraps PATCH /apps/{id_or_name}/config-vars.',
