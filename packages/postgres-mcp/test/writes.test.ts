...
): Promise<ToolResult> => client.callTool({ name, arguments: args }) as Promise<ToolResult>;
...
describe('backup write tools', () => {
  ...
  it('pg_backups_delete resolves num from b001 and DELETEs the app-scoped transfer', async () => {
    const { client, calls } = await spinUpServer({
      responses: [
        {
          match: (url, init) => url === `${V11}/apps/demo/transfers/1` && init?.method === 'DELETE',
          status: 200,
  ...
  it('pg_backups_delete resolves the owning app from the database', async () => {
    const { client, calls } = await spinUpServer({
      responses: [
        {
          match: (url) => url === addonLookup(DB_NAME),
          body: { id: DB_UUID, app: { name: 'demo' } },
        },
  ...
  it('pg_backups_schedule maps a PST abbreviation and a custom name', async () => {
    const { client, calls } = await spinUpServer({
      responses: [
        {
          match: (url) => url === `${V11}/databases/${DB_UUID}/transfer-schedules`,
          status: 201,
          body: {},
        },
      ],
    });
    await call(client, 'pg_backups_schedule', {
      database: DB_UUID,
      at: '14:00 PST',
...
describe('connection + settings write tools', () => {
  it('pg_connection_reset resolves the addon id and POSTs connection_reset', async () => {
    ...
    const result = await call(client, 'pg_connection_reset', {
      database: DB_NAME,
      confirm: DB_NAME,
    });
    expect(parseEnvelope(result).ok).toBe(true);
    expect(
      find(calls, (c) => c.url === `${V11}/databases/${DB_UUID}/connection_reset`)?.method,
    ).toBe('POST');
  });
  ...
  it('pg_maintenance_window_set POSTs {day_of_week,time_of_day} to the maintenances API', async () => {
    const { client, calls } = await spinUpServer({
      responses: [
        { match: (url) => url === addonLookup(DB_NAME), body: { id: DB_UUID, name: DB_NAME } },
        {
          match: (url, init) => url === `${MAINT}/${DB_UUID}/window` && init?.method === 'POST',
          status: 200,
