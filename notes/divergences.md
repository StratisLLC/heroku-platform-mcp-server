...
## 68. Phase 4.5 smoke test — HTTP server never refreshed expired Heroku tokens

**Observation:** Heroku OAuth access tokens have an ~8-hour TTL. The Phase 4 HTTP server stored both the access token and a refresh token (and an `expires_at` column) but never used the refresh token — `resolveUserAccessToken` loaded and decrypted the access token without checking expiry. Result: every hosted user was silently locked out ~8 hours after sign-in. The capability prober's first check (`account.self`) returned 401, aborting the whole matrix, leaving only the 6 no-probe tools visible (`whoami`, `schema_info`, `rate_limit_status`, `audit_tail`, `dynos_run`, `refresh_capabilities`). `refreshAccessToken()` already existed in `oauth/heroku.ts` but was never wired into the request path. Fixed in http-server-v0.2.2: `resolveUserAccessToken` now refreshes proactively when the token is expired or within 60s of expiry, persisting Heroku's rotated refresh token. Reactive-on-401 (defense in depth for revoked-mid-session tokens) was deferred — implementing it cleanly requires threading a refresh callback into `core/client.ts` per-call, which is too invasive for a point fix. The 60s proactive buffer covers the normal 8-hour expiry case, which is the bug that was silently locking users out. Reactive-on-401 stays on the list for a follow-up patch.

**Rule for the future:** Any stored third-party OAuth token with a TTL must have its refresh path wired into the request flow AND covered by a test that uses an expired-token fixture. Storing a refresh token without using it is worse than not storing it — it implies a capability that does not exist.

## 69. Phase 4.5 — ERR_HTTP_HEADERS_SENT on /mcp SSE responses

**Observation:** `routes/mcp.ts` handed the raw Node response to `StreamableHTTPServerTransport.handleRequest()`, which writes headers and streams SSE directly, then returned `undefined`. Hono's node-server adapter interpreted `undefined` as "no response produced" and tried to finalize the response itself, calling `writeHead` on the already-written response → `ERR_HTTP_HEADERS_SENT` spam on every MCP request (non-fatal — responses still got through — but noisy and a latent risk). Fix: return `RESPONSE_ALREADY_SENT` (exported from `@hono/node-server/utils/response`) instead of `undefined`, telling Hono the response was already handled. Not unit-tested — the bug only manifests through `@hono/node-server`'s post-handler finalize step with a real raw Node `ServerResponse`, which the route-level rig doesn't reproduce; the integration test path (real HTTP + real Postgres) exercises it implicitly.

**Rule for the future:** When an MCP SDK transport writes directly to the raw Node response, the Hono handler must return `RESPONSE_ALREADY_SENT`, never `undefined` or a fresh `Response`.

## 70. Phase 4.5 — token refresh ran once per session, not per request (completes #68)

**Observation:** v0.2.2 wired proactive refresh into `resolveUserAccessToken` but called it only at session creation (inside `if (!session)` in `routes/mcp.ts`); the resolved token string was baked into `buildSessionMcp` and reused for the life of the session. Long-lived MCP sessions (the common case — a connector left connected) therefore still hit the original ~8h lockout because the baked-in token was never re-resolved. Verified live: tool calls in an existing session did not refresh an artificially-expired token. Fix (v0.2.3): pass `getAccessToken: () => resolveUserAccessToken(...)` into `buildSessionMcp` instead of a static string. The core client already invokes its token provider per request attempt (`client.ts:329`, awaits sync-or-async), so the getter now resolves-and-refreshes on every call with zero core changes. Session creation still validates auth once for fail-fast behavior. Mid-session `ReauthRequiredError` is translated by the route's getter closure into a core `AuthError` carrying `details: { code: 'reauth_required', signInUrl }`, so the platform-mcp `runTool` envelope renders as `kind:'auth'` (not the generic `kind:'server'` that a bare `Error` would produce via `toToolEnvelope`).

**Rule for the future:** A "lazy token provider" getter must actually resolve lazily. Passing `() => capturedStaticValue` defeats the pattern. When a client exposes a per-request token callback, wire it to the live resolver, not to a value captured once. Test refresh at the REQUEST level, not just the resolver level — the v0.2.2 tests proved the function refreshes but not that it runs at the right point in the lifecycle (same class of bug as #68, one layer up). A focused unit test on the getter wiring (counting invocations through `built.context.client`) covers this without needing a full transport rig.
